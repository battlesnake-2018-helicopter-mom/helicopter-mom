...
def neighbors_of(x, y, map):
    ...
    """
    assert 0 <= x < map.shape[0], "X coordinate must be in bounds!"
    assert 0 <= y < map.shape[1], "Y coordinate must be in bounds!"

    if x + 1 < map.shape[0] and map[y][x+1] != MAP_SNAKE:
        yield x + 1, y
    if y + 1 < map.shape[1] and map[y+1][x] != MAP_SNAKE:
        yield x, y + 1
